...
const getForecast = (id) => {
  $.get(`http://api.openweathermap.org/data/2.5/forecast?id=${id}&APPID=${apikey}`)
    .then((res) => {
      ...
      let day1 = forecastArr[5]
      let day2 = forecastArr[13]
      let day3 = forecastArr[21]
      let day4 = forecastArr[29]
      let day5 = forecastArr[37]
      let forecast = [day1, day2, day3, day4, day5]
      console.log(forecast)
      $('#forecast').html('')
      forecast.forEach((day) => {
        let cardDay = $('<div>')
        let date = getDate(day.dt)
...
const getCurrentWeather = (city) => {
  $.get(`http://api.openweathermap.org/data/2.5/weather?q=${city}&APPID=${apikey}`)
    .then((weatherInfo) => {
      ...
      $('#curWind').text(`Wind Speed: ${weatherInfo.wind.speed} mph`)
